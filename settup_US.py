...
test=read_dist('dist.dat')

### Now have to sort by 0.1 nm
...
for num in test2:
    old=old+1
    trjconv('6.1.gro','pull.xtc', str(old)+'.gro',str(num),str(num))
...
for num in test2:
    old_=old_+1
    grompp('pull.mdp', str(old_)+'.gro','topol.top',str(old_)+'.tpr','leaflet.ndx')
